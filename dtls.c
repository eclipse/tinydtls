...
/**
 * Get key exchange algorithm of ciphher suite.
 * \param cipher_index index to cipher suite params
 * \return key exchange algorithm. \c DTLS_KEY_EXCHANGE_NONE, if cipher is not supported.
 */
static inline cipher_suite_key_exchange_algorithm_t get_key_exchange_algorithm(int cipher_index)
{
  assert(0 <= cipher_index && cipher_index < last_cipher_suite_param);
...
known_cipher(dtls_context_t *ctx, int cipher_index, int is_client) {
  ...
  const cipher_suite_key_exchange_algorithm_t key_exchange_algorithm = get_key_exchange_algorithm(cipher_index);
			 ...
			 uint8 *data, size_t data_length, int client_hello)
{
  ...
  if (data_length < sizeof(uint16)) {
    ...
    if (DTLS_KEY_EXCHANGE_ECDHE_ECDSA == get_key_exchange_algorithm(handshake->cipher_index)) {
      goto error;
    }
    return 0;
  }
...
dtls_update_parameters(dtls_context_t *ctx,
  ...
  while ((i >= (int)sizeof(uint16)) && !ok) {
    config->cipher_index = get_cipher_index(dtls_uint16_to_int(data));
...
dtls_prepare_record(dtls_peer_t *peer, dtls_security_parameters_t *security,
  ...
  } else { /* TLS_PSK_WITH_AES_128_CCM_8, TLS_PSK_WITH_AES_128_CCM, TLS_ECDHE_ECDSA_WITH_AES_128_CCM_8 or TLS_ECDHE_ECDSA_WITH_AES_128_CCM */
    /**
...
#define A_DATA_LEN 13
    ...
    if (mac_len == 0) {
      ...
      if (key_exchange_algorithm == DTLS_KEY_EXCHANGE_PSK) {
        dtls_debug("dtls_prepare_record(): encrypt using TLS_PSK_WITH_AES_128_CCM_%d\n", mac_len);
      } else if (key_exchange_algorithm == DTLS_KEY_EXCHANGE_ECDHE_ECDSA) {
        dtls_debug("dtls_prepare_record(): encrypt using TLS_ECDHE_ECDSA_WITH_AES_128_CCM_%d\n", mac_len);
      }
    }
				...
				uint8 *data, size_t data_length)
{
  ...
  assert(DTLS_KEY_EXCHANGE_ECDHE_ECDSA == get_key_exchange_algorithm(config->cipher_index));
...
dtls_send_server_hello(dtls_context_t *ctx, dtls_peer_t *peer)
{
  ...
  const int ecdsa = DTLS_KEY_EXCHANGE_ECDHE_ECDSA == get_key_exchange_algorithm(handshake->cipher_index);
		      ...
		      uint8 *data, size_t data_length)
{
  ...
  handshake->cipher_index = get_cipher_index(dtls_uint16_to_int(data));
			      ...
			      uint8 *data, size_t data_length)
{
  ...
  assert(DTLS_KEY_EXCHANGE_PSK == get_key_exchange_algorithm(config->cipher_index));
			  ...
			  uint8 *data, size_t data_length)
{
  ...
  assert(DTLS_KEY_EXCHANGE_ECDHE_ECDSA == get_key_exchange_algorithm(peer->handshake_params->cipher_index));
...
handle_verified_client_hello(dtls_context_t *ctx, dtls_peer_t *peer,
  ...
  if (DTLS_KEY_EXCHANGE_ECDHE_ECDSA == get_key_exchange_algorithm(peer->handshake_params->cipher_index) &&
...
handle_handshake_msg(dtls_context_t *ctx, dtls_peer_t *peer, uint8 *data, size_t data_length) {
  ...
  const cipher_suite_key_exchange_algorithm_t key_exchange_algorithm = get_key_exchange_algorithm(peer->handshake_params->cipher_index);
  ...
  case DTLS_HT_CLIENT_KEY_EXCHANGE:
    ...
    if (key_exchange_algorithm == DTLS_KEY_EXCHANGE_ECDHE_ECDSA && is_ecdsa_client_auth_supported(ctx))
