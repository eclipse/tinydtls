 ...
 * \param data_len_       array sizes of the payloads in correct order.
 ...
 * A Alert is sent to the peer (using the write callback function
 * registered with \p ctx). The return value is \c -1 on error, \c 1 when
 * the Alert is sent.
 *
 ...
 * \return \c 1 if msg is sent, or \c -1 otherwise.
 */
static int
			     ...
			     dtls_alert_t description)
{
  ...
  int res = CALL(ctx, write, ephemeral_peer->session, buf, sizeof(buf));
  ...
  return res <= 0 ? res : (int) sizeof(buf);
}
 ...
 * registered with \p ctx). The return value is \c -1 on error, \c 1 when
 * the HelloVerifyRequest is sent.
 *
 * \param ctx              The DTLS context.
 * \param ephemeral_peer   The ephemeral remote party we are talking to.
			     ...
			     uint8 *data, size_t data_length)
{
  ...
  res = CALL(ctx, write, ephemeral_peer->session, buf, sizeof(buf));
...
handle_handshake_msg(dtls_context_t *ctx, dtls_peer_t *peer, session_t *session,
  ...
  case DTLS_HT_CLIENT_HELLO:
    ...
    if (state != DTLS_STATE_CONNECTED && state != DTLS_STATE_WAIT_CLIENTHELLO) {
      return dtls_alert_fatal_create(DTLS_ALERT_UNEXPECTED_MESSAGE);
    }
     ...
     * state is left for re-negotiation of key material. */
      ...
      * the cookie exchange */
    if (state == DTLS_STATE_WAIT_CLIENTHELLO) {
...
dtls_handle_message(dtls_context_t *ctx,
  ...
  /* check for CLIENT_HELLOs of epoch 0, maybe a peer's start over */
  ...
  if (!peer) {
    ...
    /** no peer, no CLIENT_HELLO => drop it */
   return 0;
  } else {
